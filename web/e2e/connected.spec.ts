...
test("uses one SPAN map for complete journeys and keeps the build-order settings", async ({ page }) => {
  ...
  await expect(page.locator("#map .connected-package-pin")).toHaveCount(1);
  await expect(page.locator("#map .connected-end")).toHaveCount(0);
  await expect(page.locator("#connected-route-detail")).toBeHidden();
  await expect(page.locator("#map-legend-items .connected-key").nth(1)).toHaveJSProperty("hidden", true);
  expect((await new AxeBuilder({ page }).analyze()).violations).toEqual([]);
  await expect(page).toHaveURL(/extent=package/);
  await page.reload();
  await expect(page.locator("#map .connected-package-pin")).toHaveCount(1);
  await page.locator("#connected-package-list > summary").click();
  await expect(page.locator("#connected-funded-projects")).toContainText("Synthetic proposal");
  await page.getByRole("button", { name: "Inspect a journey using Synthetic proposal" }).click();
  await expect(page.locator("#map .connected-end")).toHaveCount(2);
  await expect(page.locator("#map .connected-package-pin")).toHaveCount(0);
  await expect(page.locator("#connected-route-detail")).toBeVisible();
  await expect(page.locator("#map-legend-items .connected-key").nth(1)).toHaveJSProperty("hidden", false);
  await page.locator("#connected-package-map").click();
  await page.locator("#map .connected-package-pin").click();
  await expect(page.locator("#connected-route-detail")).toBeVisible();
  await expect(page.locator("#map .connected-end")).toHaveCount(2);
  ...
  await expect(page.locator("#connected-package-map")).toBeDisabled();
